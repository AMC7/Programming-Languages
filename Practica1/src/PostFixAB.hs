...
data Comando = L Int | EXEC | ADD | SUB | MUL | DIV | REM | Eq | Gt | Lt | SEL | NGET | POP | SWAP | SEC [Comando] |LB Bool |NOT |AND deriving(Show,Eq)
...
pilaConSoloNumeros (x:xs) = case x of (L a) -> True && (pilaConSoloNumeros (xs)) 
                                      (LB b) -> True && (pilaConSoloNumeros (xs))
                                      _ -> False
...
and'::Comando->Comando->Comando
and' (LB True) (LB True) = LB True
and' (LB _) (LB _) = LB False
and' _ _= error "Error de ejecucion no se estan recibiendo literales booleanas"

not'::Comando->Comando
not' (LB x) = if (x==True) then LB False else LB True
not' _ = error "Error de ejecucion no se estan recibiendo literales booleanas"
...
opL (L x) (L y) Lt = if (x<y) then (LB True) else (LB False)
opL (L x) (L y) Eq = if (x==y) then (LB True) else (LB False)
opL (L x) (L y) Gt = if (x>y) then (LB True) else (LB False)
opL x y AND = and' x y
...
execSec (c:cs) p = case c of
  ...
  NOT -> if (length p) >=1 then (not'(head p)):(tail p) else error "Error de ejecucion no es una pila válida"  				   
...
--Devuelve L 3
ex2 = exec (POSTFIX,1,[(SEC [L 2,MUL]),EXEC]) ([L 7]) --SI
--Devuelve L 14 
ex3 = exec (POSTFIX,2,[L 2,NGET]) ([L 9 ,L 12]) --SI
--L 12
ex4 = exec (POSTFIX,1,[SEC [L 2,MUL],NGET]) ([L 3]) --SI 
--No valido
ex5 = exec (POSTFIX,2,[Lt]) ([L 2 ,L 3]) 
--LB False
ex6 = exec (POSTFIX,2,[Eq]) ([L 2 ,L 2]) 
--LB True
ex7 = exec (POSTFIX,2,[Eq]) ([L 2 ,L 3]) 
--LB False
ex8 = exec (POSTFIX,2,[AND]) ([LB True ,LB False]) 
--LB False
ex9 = exec (POSTFIX,2,[AND]) ([LB True ,LB True]) 
--LB True
ex10 = exec (POSTFIX,1,[NOT]) ([LB True]) 
--LB False
ex11 = exec (POSTFIX,1,[NOT]) ([LB False]) 
--LB True
